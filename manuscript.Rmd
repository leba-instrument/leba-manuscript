...
library(packrat)
#This chunk is disabled. Only run this to install the dependencies automatically.
packrat::init()
...
#if "packrat" does not work you can run this chunk manually to get all required packs"
...
+-------------------------------------------------+---------+--------------+----------------------------------------------------------------------------------------+----------------+
| Name                                            | Authors | Translations | Description                                                                            | Relevant Items |
+=================================================+=========+==============+========================================================================================+================+
| Visual Light Sensitivity Questionnaire-8        |         |              | Eight-question survey to assess the presence and severity of photosensitivity symptoms |                |
+-------------------------------------------------+---------+--------------+----------------------------------------------------------------------------------------+----------------+
| Office Light Survey                             |         |              | To assess electrical lighting environment in Office                                    |                |
+-------------------------------------------------+---------+--------------+----------------------------------------------------------------------------------------+----------------+
| Harvard Light Exposure Assessment Questionnaire |         |              | Self-administered semiquantitative light questionnaire                                 |                |
+-------------------------------------------------+---------+--------------+----------------------------------------------------------------------------------------+----------------+
| Hospital Lighting Survey                        |         |              | 23 items questionnaire to assess light environment in a hospital                       |                |
+-------------------------------------------------+---------+--------------+----------------------------------------------------------------------------------------+----------------+
| Morningness-Eveningness Questionnaire           |         |              | 19 items questionnaire to understand your body clock                                   |                |
+-------------------------------------------------+---------+--------------+----------------------------------------------------------------------------------------+----------------+
| Munich Chronotype Questionnaire (MCTQ)          |         |              | 17 items questionnaire to understand individuals phase of entrainment                  |                |
+-------------------------------------------------+---------+--------------+----------------------------------------------------------------------------------------+----------------+
| Assessment of Sleep Environment                 |         |              | 13 items questionnaire measuring your sleep environment quality                        |                |
+-------------------------------------------------+---------+--------------+----------------------------------------------------------------------------------------+----------------+
| The Pittsburgh Sleep Quality Index (PSQI)       |         |              | 9 items inventory to measure sleep quality and sleeping pattern                        |                |
+-------------------------------------------------+---------+--------------+----------------------------------------------------------------------------------------+----------------+
|                                                 |         |              |                                                                                        |                |
+-------------------------------------------------+---------+--------------+----------------------------------------------------------------------------------------+----------------+
|                                                 |         |              |                                                                                        |                |
...
```{r figCor,out.width="70%",out.width="100%",fig.cap= "Correlation plot of the items",results='asis', fig.align='center'}
...
parallel = ggplot(Adjusted.EV, aes(x=num, y=eigenvalue, shape=type, color=type)) +
  ...
  scale_y_continuous(name="Eigen Value", limits=c(0, 10), breaks=seq(0,10,1)) +
...
```{r Hull, include=FALSE}
# HULL
hullEFA(EFA.data,extr = "ULS", index_hull = "CAF", display = TRUE, graph = T,
        details = TRUE)
hull <- ggplot2::last_plot()
Hull <- hull+ ggtitle(NULL) +xlab("Factor Number")+ylab("CAF")+ theme(panel.grid.major = element_blank(),
                                               panel.grid.minor = element_blank(),
                                               panel.background = element_blank(),
                                               axis.line.x = element_line(color='black'),
                                               axis.line.y = element_line(color='black'))
...
Scree plot ( Figure \@ref(fig:facIdFig)) suggested a six-factor solution. Horn's parallel analysis [@hornRationaleTestNumber1965] with 500 iterations also indicated a six-factor solution. However, the MAP method [@velicerDeterminingNumberComponents1976] and Hull method [@lorenzo-sevaHullMethodSelecting2011] suggested a five-factor solution. As a result, we tested both five-factor and six-factor solutions.

```{r facIdFig, fig.align='center', fig.cap='Factor Identification (A) Parallel analysis (B) Scree Plot (C) Hull Method',  warning=FALSE}
cowplot::plot_grid(parallel, scree.plot,Hull, labels = c('(A)', '(B)', '(C)'), label_size = 8)
...
apa_table(factor.des,align = "c", caption = "Factor loadings and communality of the retained items", note = "Only loading higher than .30 is reported", font_size = "tiny")
```

```{r Residuals, out.height= "50%",  fig.align='center', fig.cap= ' Histogram of residulas:  five-factor solution',  out.width="50%",  warning=FALSE, results='asis'}
...
## Developing the short form of LEBA
...
To develop a shorter form of the LEBA we sought to Item response theory.IRT complements the conventional classical test theory-based analysis by gathering information on item discrimination and item difficulty[@bakerBasicsItemResponse2017]. Here, an item's quality is judged based on item information in relation to participants' latent trait level ($\theta$). We gathered evidence on item quality by fitting each factor of LEBA with the graded response model to the combined EFA sample and CFA sample (n =690)(\@ref(tab:IRTparameters). Item discrimination indicates the pattern of variation in the categorical responses with the changes in latent trait, and item information curve (IIC) indicates the amount of information an item carries along the latent trait continuum. Currently, apart from some general suggestions, there exists no definitive rule to judge the item discrimination and item information parameter quality. Here, we reported the item discrimination parameter and only discarded the items with relatively flat item information curve (information \<.2) to develop the short form of LEBA. @bakerBasicsItemResponse2017 categorized the item discrimination in as none = 0; very low =0.01 to 0.34; low = 0.35 to 0.64; moderate = 0.65 to 1.34 ; high = 1.35 to 1.69; very high \>1.70. Item discrimination parameters of our scale fell in very high (10 items), high (4 items), moderate (4 items), low ( 5 items) indicating a good range of discrimination along the latent trait. Examination of the item information curve indicated 6 items (1, 25, 9, 38, 30, & 41) had relatively flat information curves thus discarded.
...
knitr::include_graphics('Manuscript.figures/Item Informationjpg.jpg')
...
F1_info <- plot(F1_fit, type = "infoSE", main = "Blue Filter") 

F2_info.redu <- plot(F2_fit.redu, type = "infoSE", main = "Natural Light Test Information")


F3_info.redu <- plot(F3_fit.redu, type = "infoSE", main = "Smart Device Test Information")

F4_info.redu <- plot(F4_fit.redu, type = "infoSE", main = "Sleep Environment Test Information")

F5_info <- plot(F5_fit, type = "infoSE", main = "Electric Light Test Information")



Test_Info <- plot_grid(F1_info, F2_info.redu,F3_info.redu,F4_info.redu,F5_info, 
          labels = c('A', 'B','C', 'D', 'E' ), label_size = 10)
