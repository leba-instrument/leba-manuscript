...
data <- readRDS("leba_2021-09-03.rds")
...
```{r FlowchartFig,  out.width = '100%', out.height= '100%', fig.align = 'center', fig.cap='Development and psychometric properties of LEBA '}
...
# HULL
hullEFA(EFA.data,extr = "ULS", index_hull = "CAF", display = TRUE, graph = TRUE,
        details = TRUE)
...
```{r factor_identification_fig, out.height= "50%",  fig.align='center', fig.cap='Factor Identification (A) Parallel analysis (B) Scree Plot, (C) Hull method', fig.show="hold",fig.ncol = 2, out.width="50%",  warning=FALSE}
knitr::include_graphics('parallel.png')
scree(correlations$rho,factors=TRUE,pc=TRUE,main="(B)",
      hline=NULL,add=F)
knitr::include_graphics('HUll method.jpeg')
...
CFa.matrix[1,] = c("Five factor model", 
...
CFa.matrix[2,] = c("Five factor model", 
...
```{r Invarianceshortcut}
#library("semTools")
invariance.model <- measurementInvariance(model = LB.model.Cor.2,
                                          data = invariance.data,
                                          group = "English_Speaking",
                                          ordered =   names(CFA.data),
                                          estimator = "WLSMV",
                                          strict = T)
invariance.model$fit.configural
```
...
LB.model.Cor.3 <- "F1 =~ a1*item16 + a2*item36 + item17
                 F2 =~ item11 + item10 + item12+ item7+ Ritem8+ item9
                 F3 =~  item27+ item3+ item40 + item30 + item41
                 F4 =~ a4*item32 +a3*item35 + item38+ item33 
                 F5 =~ item46+ item45+ item25+ item4+ item1 
                       a1==a2
                       a4==a3
                item30 ~~  item41
...
summary(strucrural, fit.measures =TRUE,standardized = TRUE, rsq =TRUE)
...
Invariance.table <- compareLavaan(models,
              ...
              fitmeas = c("chisq", "df", "pvalue", "cfi.scaled","tli.scaled","rmsea.scaled", "rmsea.ci.lower.scaled", "rmsea.ci.upper.scaled","srmr" ),
              scaled = TRUE,
              chidif = TRUE, digits = 2)
...
F1.IRT.data <- sem.data %>% 
  select(item16, item36,  item17)

F2.IRT.data <- sem.data %>% 
  select(item16, item36,  item17)

"F1 =~ item16 + item36 + item17
                 F2 =~ item11 + item10 + item12+ item7+ Ritem8+ item9
                 F3 =~  item27+ item3+ item40 + item30 + item41
                 F4 =~ a1*item32 +item35 + item38+ a2*item33 
                 F5 =~ item46+ item45+ item25+ item4+ item1


IRTRitem8 = recode(IRT.data$item8, '1=5; 2=4; 3=3; 4=2; 5=1')

IRT.data <- IRT.data %>% 
  mutate(Ritem8 = IRTRitem8, Ritem37 = IRTRitem37)
```
