...
```{r dependencymanager, include = FALSE}
library(packrat)
packrat::init()
```
...
export_svg(Flowchart1) %>%
  charToRaw() %>%
  rsvg() %>%
  png::writePNG("mFlowchart1.png")

```



```{r FlowchartFig, fig.align='center', fig.cap='Development', include=FALSE, out.height='100%', out.width='100%'}
...
export_svg(EFAplot.LR) %>%
  charToRaw() %>%
  rsvg() %>%
  png::writePNG("Manuscript.figures/EFAplot.LR.png")

```

```{r EFAplot, fig.align='center', fig.cap='Five Factor Solution', warning=FALSE, include=FALSE, out.height="100%", out.width="120%"}
knitr::include_graphics('Manuscript.figures/EFAplot.LR.png')
...
```{r Invarianceshortcut, include=FALSE}
#library("semTools")
