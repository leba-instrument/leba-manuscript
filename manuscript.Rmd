...
pacman::p_load(MOTE, tidyverse, psych, lavaan,kableExtra,gt,gtsummary, mirt,likert,kutils,semPlot,semTable,semTools,ggcorrplot,dlookr,qgraph,paran,EFA.MRFA,VIM,DiagrammeR,DiagrammeRsvg,ggplot2,cowplot,rsvg,questionr,magick, simsem,readxl, stringr,RColorBrewer,WrightMap)
...
Flowchart1 <- grViz("digraph nicegraph{
                    ...
                    graph [fontname = Helvetica,layout = dot,fontsize = 40, ]
                     
                     node[fontname = Helvetica, shape = rectangle, margin = 0.2, width = 15, height = 1, 
                    fixedsize = true,fontsize = 40]  
                     
                    A[label = '48 items were generated by a robust expert panel'] 
                    B[label = 'Exploratory Factor Analysis on 48 Items (N = 428)']
                    
                    node[fontname = Helvetica, shape = rectangle, margin = 0.2, width = 15, height = 6, 
                    fixedsize = true, fontsize = 40]
                    C[label = 
                    <
                    ...
                    >
                     ...
                     subgraph cluster_1 {
                      ...
                      style = rounded
                     node[fontname = Helvetica, margin = 0.2,width = 18, height = 2, 
                    fixedsize = true, fontsize = 38 ]
                    
                     1[label = '5 Factor Solution Retained 25 Items']
                     2[label = 'Reliability Analysis on EFA Sample (N=428)
                    Ordinal alpha F1 =.94; F2 = .76; F3 = .75; F4 =.72; F5 =.62']
                    ...
                    4[label = 'Confirmatory Factor Analysis 
                     ...
                     5[label = 'Reliability Analysis on CFA (23 Items; N=262) 
                      Ordinal alpha F1=.93; F2=.83; F3 =.70; F4=.69; F5=.52
                      Mcdonald  omega(total) for the total scale = .73']
                    8[label = 'Measurement Invariance Analysis
                    Residual measurement invariance achieved']
                    9[label = 'Semantic Analysis']
                    10[label = 'LEBA Short Form (17 Items; N=690) 
...
export_svg(Flowchart1) %>%
  charToRaw() %>%
  rsvg() %>%
  png::writePNG("Manuscript.figures/Flowchart1.png", dpi = 1200)
...
```{r FlowchartFig, echo=FALSE, fig.align='center', fig.cap='Development of long and short form of LEBA', out.height='160%', out.width='100%'}
...
Hull <- hull+ ggtitle(NULL) +xlab("Factor Number (Hull Method)")+ylab("CAF")+ 
  apatheme +scale_color_manual(labels = c("Out side the convex Hull", "On the convex Hull"), values = c("red", "cyan2"))+theme(legend.position = "None") 
```
...
```{r facIdFig, out.height="350%", out.width="100%",fig.align='center', fig.cap='Factor Identification (A) Parallel analysis (B) Scree Plot (C) Hull Method',  warning=FALSE, results= 'asis'}
...
export_svg(EFAplot) %>%
  charToRaw() %>%
  rsvg() %>%
  png::writePNG("Manuscript.figures/EFAplot.png", dpi = 1200)
...
```{r EFAplot, fig.align='center', fig.cap='Five Factor Solution', warning=FALSE, out.height="60%", out.width="100%", results='asis'}
...
```{r MIdes, include=FALSE}

midata <- CFA.descriptives[,c(1:2,5:7)]
colnames(midata) <- c("Age", "Gender",  "Native_Speaker", "Work/School", "Mode" )
mides <- midata%>%
  tbl_summary(
    by = Native_Speaker,
    statistic = list(all_continuous() ~ "{mean} ({sd})",
...
as_tibble(mides) -> mides_tibble 
as_kable_extra(mides, format = "latex",booktabs = T) -> mides_kable #save it as a knitr::kable
...
## Semantic Analysis 
To find out if our developed tool is overlapping with existing instruments, we subjected the items of LEBA to the "Semantic Scale Network"(SSN) analysis [@rosenbusch2020semantic]. The SSN detects semantically related scales and provides cosine similarity index ranging between -.66 to 1 [@rosenbusch2020semantic]. Pair of scales with a cosine similarity index value of 1 indicates they are perfectly semantically similar scales indicating redundancy. LEBA  appeared most strongly related to scales about sleep: "Sleep Disturbance Scale For Children" [@bruni1996sleep] and "WHO-Composite International Diagnostic Interview (CIDI): Insomnia"[@who1990].The cosine similarities lie between .47 to .51. Two factors of our LEBA tool: "Using phone and smart-watch in bed" and "Using light before bedtime" dealt with light exposure related behavior pertaining to sleep quality. As such the similarity index obtained is expected.
...
```{r saveiteminfo, eval=FALSE, include=FALSE}
jpeg("Manuscript.figures/iteminfo.png", width = 800, height = 750)
plot_grid(F1_iteminfo, F2_iteminfo,F3_iteminfo,F4_iteminfo,F5_iteminfo, 
          labels = "AUTO", label_size = 10,align = "v",ncol = 2)

dev.off()
```

```{r IRTreliability, include=FALSE}
...
```{r iteminfo, include=F, warning=F}
F1_item1<- plot(F1_fit, type = "infotrace", 
                    facet_items = T, which.items = 1, main = "")

F1_item2 <- plot(F1_fit, type = "infotrace", 
     facet_items = T, which.items = 2,main = "")

F1_item3 <- plot(F1_fit, type = "infotrace", 
     facet_items = T, which.items = 3,main = "")


F2_item1 <- plot(F2_fit, type = "infotrace", facet_items = T, which.items = 1,main = "")
F2_item2 <- plot(F2_fit, type = "infotrace", facet_items = T, which.items = 2,main = "")
F2_item3 <- plot(F2_fit, type = "infotrace", facet_items = T, which.items = 3,main = "")
F2_item4 <- plot(F2_fit, type = "infotrace", facet_items = T, which.items = 4,main = "")
F2_item5 <- plot(F2_fit, type = "infotrace", facet_items = T, which.items = 5,main = "")
F2_item6 <- plot(F2_fit, type = "infotrace", facet_items = T, which.items = 6,main = "")

F3_item1 <- plot(F3_fit, type = "infotrace", 
                    facet_items = T, which.items = 1,main = "")
F3_item2 <- plot(F3_fit, type = "infotrace", 
                    facet_items = T, which.items = 2,main = "")

F3_item3 <- plot(F3_fit, type = "infotrace", 
                    facet_items = T, which.items = 3,main = "")
F3_item4 <- plot(F3_fit, type = "infotrace", 
                    facet_items = T, which.items = 4,main = "")
F3_item5 <- plot(F3_fit, type = "infotrace", 
                    facet_items = T, which.items = 5,main = "")



F4_item1 <- plot(F4_fit, type = "infotrace", facet_items = T,which.items = 1,main = "")
F4_item2 <- plot(F4_fit, type = "infotrace", facet_items = T,which.items = 2,main = "")
F4_item3 <- plot(F4_fit, type = "infotrace", facet_items = T,which.items = 3,main = "")
F4_item4 <- plot(F4_fit, type = "infotrace", facet_items = T,which.items = 4,main = "")


F5_item1 <- plot(F5_fit, type = "infotrace", facet_items = T,which.items = 1,main = "")
F5_item2 <-plot(F5_fit, type = "infotrace", facet_items = T,which.items = 2,main = "")
F5_item3 <-plot(F5_fit, type = "infotrace", facet_items = T,which.items = 3,main = "")
F5_item4 <-plot(F5_fit, type = "infotrace", facet_items = T,which.items = 4,main = "")
F5_item5 <-plot(F5_fit, type = "infotrace", facet_items = T,which.items = 5,main = "")
...
F3title <- ggdraw() + 
  draw_label(
    "F3: Using phone and smart-watch in bed",
    fontface = 'bold',
    x = 0,
    hjust = 0
  ) +
...
f1itemplot <- plot_grid(F1_item1, F1_item2,F1_item3,
          labels = "AUTO", label_size = 10,align = "v")
f2itemplot <- plot_grid(F2_item1, F2_item2,F2_item3,F2_item4,F2_item5,F2_item6,
           label_size = 10,align = "v")
f3itemplot <- plot_grid(F3_item1, F3_item2,F3_item3,F3_item4,F3_item5,
           label_size = 10,align = "v")
f4itemplot <- plot_grid(F4_item1, F4_item2,F4_item3,F4_item4,
          labels = "AUTO", label_size = 10,align = "v")

f5itemplot <- plot_grid(F5_item1, F5_item2,F5_item3,F5_item4,F5_item5,
          label_size = 10,align = "v")
```

```{r f1itminf, include=F, warning=F}

jpeg("Manuscript.figures/F1testinfo.png", width = 800, height = 750)
...
```{r f2iteminf, include=F, warning=F}
jpeg("Manuscript.figures/F2testinfo.png", width = 800, height = 750)
plot_grid(
  F2title, f2itemplot,
  ncol = 1,
  # rel_heights values control vertical title margins
...
```{r f3iteminf, include=F, warning=F}
jpeg("Manuscript.figures/F3testinfo.png", width = 800, height = 750)
plot_grid(
  F3title, f3itemplot,
  ncol = 1,
  # rel_heights values control vertical title margins
...
```{r f4iteminf, include=F, warning=F}
jpeg("Manuscript.figures/F4testinfo.png", width = 800, height = 750)
plot_grid(
  F4title, f4itemplot,
  ncol = 1,
  # rel_heights values control vertical title margins
...
```{r f5iteminf, include=F, warning=F}
jpeg("Manuscript.figures/F5testinfo.png", width = 800, height = 750)
plot_grid(
  F5title, f5itemplot,
  ncol = 1,
  # rel_heights values control vertical title margins
...
```{r itmeinfo, eval=FALSE, fig.cap="Item information curves (a) Wearing blue light filters (b) Spending time outdoors (c) Using phone and smartwatchin bed (d) Using light before bedtime (e) Using light in the morning andduring daytime", warning=FALSE, include=FALSE, out.height="60%", out.width="100%"}
knitr::include_graphics('Manuscript.figures/iteminfo.png')
...
F1.person <- ggplot(F1.personfit, aes(x=Zh)) + geom_histogram(binwidth=.5)+geom_vline(xintercept = -2)+
  labs( y="Number of Participants", x = "Zh Statistics: F1")+apatheme
...
F2.person <- ggplot(F2.personfit, aes(x=Zh)) + geom_histogram(binwidth=.4)+
  ...
  labs( y="Number of Participants", x = "Zh Statistics: F2")+apatheme
...
F3.person <- ggplot(F3.personfit, aes(x=Zh)) + geom_histogram(binwidth=.5)+geom_vline(xintercept = -2)+
  labs( y="Number of Participants", x = "Zh Statistics: F3")+apatheme
...
F4.person <- ggplot(F4.personfit, aes(x=Zh)) + geom_histogram(binwidth=.5)+geom_vline(xintercept = -2)+
  labs( y="Number of Participants", x = "Zh Statistics: F4")+apatheme
...
F5.person <- ggplot(F5.personfit, aes(x=Zh)) + geom_histogram(binwidth=.4)+geom_vline(xintercept = -2)+
  labs( y="Number of Participants", x = "Zh Statistics:F5")+apatheme
...
```{r savetestinfo, include=FALSE, warning=F, message=F}
jpeg("Manuscript.figures/testinfo.png", width = 800, height = 750)
plot_grid(F1_test.info , F2_test.info.redu,F3_test.info.redu,F4_test.info.redu,F5_test.info,
          labels = "AUTO", label_size = 10, align = "v" )
dev.off()
```

```{r Testinfo, fig.cap="Test information curves (a) Wearing blue light filters (b) Spending time outdoors (c) Using phone and smartwatchin bed (d) Using light before bedtime (e) Using light in the morning andduring daytime", warning=FALSE, out.width="100%", out.height="60%"}

knitr::include_graphics('Manuscript.figures/testinfo.png')
```
...
Table \@ref(tab:itemfittab) summarizes the item fit indexes of the items. All the items fitted well to the respective models as assessed by  RMSEA value obtained from Signed-$\chi^2$ index implementation. All of the items had RMSEA value $\le$.06 indicating adequate fit. Figure \@ref(fig:personfit) depicts the person fit of out fitted models. Person fit indicates the validity and meaningfulness of the fitted model at the participants latent trait level [@desjardinsHandbookEducationalMeasurement2018]. We estimated the person fit statistics using standardized fit index Zh statistics [@drasgow1985appropriateness]. Zh < -2 should be considered as a misfit. Fig indicates that Zh is larger than -2 for most participants, suggesting a good fit of the selected IRT models. 
...
LEBA.Total <- LEBA.Total %>% 
  rowwise() %>% 
  mutate(F2.Total = sum(item11,  item10 , item12, item7, Ritem8, item9))
...
```{r WP, fig.cap="Person Item Map", warning=FALSE, out.width="100%", out.height="120%"}
...
48 items were generated by an expert panel and  administered to a large sample (n = `r nrows(EFA.descriptives)` to explore the latent structure. Exploratory factor analysis revalied a five factor solution with 25 items. ("Wearing blue light filters", "Spending time outdoors", "Using phone and smart-watch in bed", "Using light before bedtime",  and "Using light in the morning and during daytime"). These five factors dealt with different light exposure related behavior. The internal consistency reliability coefficient ordinal alpha ranged between `r apa(range(alpha.tab),2,F)`. As all the retained factors were meaningful and contributed essentially towards our aim we retained all five factors.


A CFA on a separate sample ((n = `r nrows(CFA.descriptives)` gave a five-factor solution (CFI =`r apa(fit.2[5],2,F)`; TLI = `r apa(fit.2[6],2,F)`); RMSEA = `r apa(fit.2[7],2,F)`[`r apa(fit.2[8],2,F)`-`r apa(fit.2[9],2,F)`, 90% CI]) and SRMR = `r apa(fit.2[10],2,F)`) after discarding two item. The internal consistency McDonald's $\omega_t$ of the five factors were  satisfactory (`r apa(rel.k[2,1],2,F)`, `r apa(rel.k[2,2],2,F)`, `r apa(rel.k[2,3],2,F)`, `r apa(rel.k[2,4],2,F)`, `r apa(rel.k[2,5],2,F)`) Internal consistency reliability of the total scale (23 item) was also satisfactory, Mcdonald's $\omega_t$= `r apa(rel.k[5,6],2,F)`.  

Lastly, we developed a short-LEBA (n=23) using IRT analysis. We fitted a graded response model model to the combined efa and cfa sample ( n =`r nrows(data)`). We discarded 5 items with relatively flat item information curve [I($\theta$) <.20]. No item was identified as a misfit item.  In terms of item difficulty, our scale contained very high  high , moderate , and low discriminating items and covered a substantial range of underlying  attributes.  Test information curve also indicated a good coverage of underlying trait continuum with precision. 
## Conclusion
"The Light exposure behavior assessment"(LEBA) gave a five solution with 25 items in an exploratory factor analysis. A confirmatory factor analysis with this 25-item scale again offered a five-factor solution, but this time two more item was discarded. The 25-item "Bangla version of Rotter’s I-E scale"LEBA" was found reliable and valid. A short-form of LEBA was developed using IRT analysis. IRT analysis gave a 18 item scale with  a good range of covergange across the underlying trait continuum. All-in-all, we can recommend both forms to be used to capture individual’s light exposure related behavior
