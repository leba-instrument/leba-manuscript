...
pacman::p_load(MOTE, tidyverse, psych, lavaan,kableExtra,gt,gtsummary, mirt,likert,kutils,semPlot,semTable,semTools,ggcorrplot,dlookr,qgraph,paran,EFA.MRFA,VIM,DiagrammeR,DiagrammeRsvg,ggplot2,cowplot,rsvg,questionr,magick, simsem,readxl, stringr)
...
```{r savetestinfo, include=F, warning=F, message=F}
...
```{r}
library(WrightMap)
wrightMap(F1.estimation,F1_items.redu[,2])
wrightMap(F2.estimation,F2_items.redu[,2])
wrightMap(F3.estimation,F3_items.redu[,2])
wrightMap(F4.estimation,F3_items.redu[,2])
wrightMap(F5.estimation,F5_items.redu[,2])
```
