...
```{r depMan, eval=FALSE, include=FALSE}
library(packrat)
...
|-------------------------------------------------|-----------------------------|----------------------------------------------------------------------------------------|:--------------:|-------------|
| Visual Light Sensitivity Questionnaire-8        | [\@verriotto2017new]        | Eight-question survey to assess the presence and severity of photosensitivity symptoms |      all       | jhghjgjhgfd |
| Office Light Survey                             | [\@eklund1996development]   | Multi-item questionnaire to assess electrical lighting environment in office           |       aa       | gh          |
| Harvard Light Exposure Assessment Questionnaire | [\@bajaj2011validation]     | Self-administered semiquantitative light qestionnaire                                  |       aa       | as          |
| Hospital Lighting Survey                        | [\@dianat2013objective]     | 23 items questionnaire to assess light environment in a hospital                       |       dd       | fg          |
| Morningness-Eveningness Questionnaire           | [\@horne1976self]           | 19 items questionnaire to understand your body clock                                   |       ff       | hj          |
| Munich Chronotype Questionnaire (MCTQ)          | [\@roenneberg2003life]      | 17 items questionnaire to understand individuals phase of entrainment                  |       ff       | jk          |
| Assessment of Sleep Environment                 | [\@grandner2014development] | 13 items questionnaire measuring your sleep environment quality                        |       ff       | kl          |
| The Pittsburgh Sleep Quality Index (PSQI)       | [\@buysse1989pittsburgh]    | 9 items inventory to measure sleep quality and sleeping pattern                        |       ff       | klfrgfgvfg  |
|                                                 |                             |                                                                                        |                |             |
|                                                 |                             |                                                                                        |                |             |
...
#as_kable_extra(desc_table) -> desc_kable_extra
```
...
apa_table(factor.des,align = "c", caption = "Factor loadings and communality of the retained items", note = "Only loading higher than .30 is reported", font_size = "small")
```
...
F1_item.info <- plot(F1_fit, type = "infotrace", main = "F1") 

F2_item.info.redu <- plot(F2_fit.redu, type = "infotrace", main = "F2")


F3_item.info.redu <- plot(F3_fit.redu, type = "infotrace", main = "F3")

F4_item.info.redu <- plot(F4_fit.redu, type = "infotrace", main = "F4")

F5_item.info <- plot(F5_fit, type = "infotrace", main = "F5")
...
plot_grid(F1_item.info, F2_item.info.redu,F3_item.info.redu,F4_item.info.redu,F5_item.info, 
