...
colnames(descriptives) <- c("Items", "Mean", "SD", "Skew", "Kurtosis", "SW$^{1}$", "Item Total Correlation")
...
Sampling adequacy was checked using Kaiser-Meyer-Olkin (KMO) measures of sampling adequacy [@kaiserIndexFactorialSimplicity1974] . The overall KMO vale for 48 items was `r printnum(KMO$MSA)` which was above the cutoff value (.50) indicating adequate sample size [@hutchesonMultivariateSocialScientist1999].  Bartlett's test of sphericity [@bartlettNoteMultiplyingFactors1954], $\chi^2$ (`r bartlet$df`) = `r apa(bartlet$chisq, 2, T)`, p < .001 indicated the correlations between items are adequate for conducting the EFA. However only `r apa(cor.per, 2,T)`% of the inter-item correlation coefficients were greater than |.30|. The inter-item correlation coefficients ranged between `r min.cor` to `r max.cor`. Figure \@ref(fig:figCor) depicts the correlation matrix.
...
#This chunk holds codes for creating 'gtExtra' based descriptive figures (EFA descriptives
...
```{r cfagt,include=F}
#This chunk holds codes for creating 'gtExtra' based descriptive figures (CFA Descriptives)
...
```{r efagtPic, echo=FALSE, out.height="100%", out.width= '100%', fig.cap= ' Summary descriptives and response pattern of EFA sample'}
knitr::include_graphics('Manuscript.figures/efagt.png',dpi =600)

```

```{r cfagtPic, echo=FALSE, out.height="100%", out.width= '100%', fig.cap= ' Summary descriptives and response pattern of CFA sample'}
