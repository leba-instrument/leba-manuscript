...
parallel = ggplot(Adjusted.EV, aes(x=num, y=eigenvalue, shape=type, color=type)) +
  ...
  geom_hline(yintercept=1, linetype = 'dashed') + apatheme +theme(text = element_text(size = 25),legend.position = "None") 
...
Hull <- hull+ ggtitle(NULL) +xlab("Factor Number (Hull Method)")+ylab("CAF")+ 
  apatheme +scale_color_manual(labels = c("Out side the convex Hull", "On the convex Hull"), values = c("red", "cyan2"))+theme(text = element_text(size = 25))  
```
...
```{r itmeinfo, fig.cap="Item information curves (a) Wearing blue light filters (b) Spending time outdoors (c) Using phone and smartwatchin bed (d) Using light before bedtime (e) Using light in the morning andduring daytime", warning=FALSE, out.width="100%", out.height="60%"}
...
```{r WP, fig.cap="Person Item Map (a) Wearing blue light filters (b) Spending time outdoors (c) Using phone and smartwatchin bed (d) Using light before bedtime (e) Using light in the morning andduring daytime", warning=FALSE, out.width="100%", out.height="120%"}
library(WrightMap)
wrightMap(estimation, threshold, 
          item.prop = 0.65,
...
papaja::render_appendix('Supplymentary.Rmd')
```


```{r echo = FALSE, results = 'asis', cache = FALSE}
papaja::render_appendix('instrument.Rmd')
```
