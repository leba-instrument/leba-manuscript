...
knitr::opts_chunk$set(echo = FALSE, warning = F, message = F, dpi=1200, fig.width = 6,
 fig.asp = 0.8,out.width = "80%")
...
export_svg(Flowchart1) %>%
  charToRaw() %>%
  rsvg() %>%
  writeJPEG("Manuscript.figures/Flowchart1.jpeg",bg = "white", quality = 1)
...
knitr::include_graphics('Manuscript.figures/Flowchart1.jpeg')
```
...
export_svg(EFAplot) %>%
  charToRaw() %>%
  rsvg() %>%
  writeJPEG("Manuscript.figures/EFAplot.jpeg",bg = "white", quality = 1)
...
residual.plot <- ggplot(residual.data, aes(x=Residuals)) + geom_histogram(binwidth=.04, col=I("black"),fill=I("grey")) + xlim(-.2,.2)+ ylim(0,250)+apatheme +geom_vline(xintercept = .05)+
  labs( y="Frequency")
...
img1 <- readJPEG("Manuscript.figures/EFAplot.jpeg")
...
```{r EFAplot, fig.align='center', fig.cap='(A) Five Factor Solution obtained in Exploratory Factor Analysis (B) Histogram of nonredundent residula correlations', warning=FALSE, out.height="80%", out.width="100%", results='asis'}
EFAplot.1
...
knitr::include_graphics('Manuscript.figures/efagt.png')

```

```{r cfagtPic, echo=FALSE, out.height="100%", out.width= '100%', fig.cap= ' Summary Descriptives of CFA Sample'}
knitr::include_graphics('Manuscript.figures/cfagt.png')
...
F1.iteminfo.1 <- ggplot(F1.item.info.data , aes(x=Theta, y=F1.item.info.1)) +
  geom_line()  +apatheme +labs(y="Item Information", title = "F1 item16") + geom_hline(yintercept = .20, colour ="red")
F1.iteminfo.2 <- ggplot(F1.item.info.data , aes(x=Theta, y=F1.item.info.2)) +
  geom_line()  +apatheme +labs(y="Item Information", title = "F1 item36")+geom_hline(yintercept = .20, colour ="red")
F1.iteminfo.3 <- ggplot(F1.item.info.data , aes(x=Theta, y=F1.item.info.3)) +
  geom_line()  +apatheme +labs(y="Item Information", title = "F1 item17")+ylim(0,6)+geom_hline(yintercept = .20, colour ="red")

###F2

F2.iteminfo.1 <- ggplot(F2.item.info.data , aes(x=Theta, y=F2.item.info.1)) +
  geom_line()  +apatheme +labs(y="Item Information", title = "F2 item11")+ylim(0,3)+geom_hline(yintercept = .20, colour ="red")

F2.iteminfo.2 <- ggplot(F2.item.info.data , aes(x=Theta, y=F2.item.info.2)) +
  geom_line()  + apatheme +
  labs(y="Item Information", title = "F2 item10")+
  ylim(0,3)+geom_hline(yintercept = .20, colour ="red")
...
F4.iteminfo.4 <- ggplot(F4.item.info.data , aes(x=Theta, y=F4.item.info.4)) +
  geom_line()  +apatheme +
  labs(y="Item Information",title = "F4 item33")+geom_hline(yintercept = .20, colour ="red")
...
F1.tic <- ggplot(F1.test.data, aes(x=Theta, y=F1.T1)) +
  geom_line()+apatheme+
  labs(y="Test Information", title = "F1 Test Inforfamtion")
...
knitr::include_graphics('Manuscript.figures/TIC.png')
```
