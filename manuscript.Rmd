...
CFA.data <- CFA.data %>% 
  mutate(Ritem8 = CFARitem8, Ritem37 = CFARitem37)

### IRT data
IRT.data <- sem.data
IRTRitem8 = recode(IRT.data$item8, '1=5; 2=4; 3=3; 4=2; 5=1')
IRTRitem37 = recode(IRT.data$item37, '1=5; 2=4; 3=3; 4=2; 5=1')
IRT.data <- IRT.data %>% 
...
EFA.descriptives #Only EFA
CFA.descriptives #Only CFA  
...
colnames(Des.combined) = c("Mean", "SD", "Skew", "Kurtosis", "Shapiro-Wilk Statistics", 
...
# Calculating the percentange of correlations higher than .30
...
#EFA.factor <- sapply(EFA.data, as.factor)
#correlations <- polychoric(EFA.factor, correct = 0)
...
```{r CFA 23 items, eval=FALSE, include=FALSE}

#CFA.data <- sem.data[429:690,]

Ritem44 = recode(CFA.data$item44, '1=5; 2=4; 3=3; 4=2; 5=1')
Ritem21 = recode(CFA.data$item44, '1=5; 2=4; 3=3; 4=2; 5=1')

CFA.data <- CFA.data %>% 
  mutate(Ritem44 = Ritem44, Ritem21 = Ritem21)


LB.model.Cor <- "F1 =~ item43 + item26 + item32
                 F2 =~ item10 + item47 + item36 + Ritem44 + item35 + item13
                 F3 =~  item14 + item41+ item7 + item27 + Ritem21
                 F4 =~ item19 + item34 + item2
                 F5 =~ item5 + item24 + item30 + item1 + item16 + item28" 



fit.Cor <- cfa(LB.model.Cor, data = CFA.data, estimator = "MLR", mimic = "Mplus", check.gradient = FALSE) 
...
```{r CFA23withMI, eval=FALSE, include=FALSE}
...
## Fit the model 
fit.Cor.3 <- cfa(LB.model.Cor.3, data = CFA.data, estimator = "MLR", mimic = "Mplus") 
...
```{r CFI18items, eval=FALSE, include=FALSE}
