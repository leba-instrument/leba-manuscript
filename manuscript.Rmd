...
relevant.scale.data %>% 
apa_table(landscape =T,caption ="List of instruments measuring related consttucts to LEBA", format = "latex",escape = T, font_size = "footnotesize", align = c("p{3cm}", rep("p{3.6cm}", ncol(relevant.scale.data)))) 
...
Flowchart1 <- grViz("digraph my_flowchart{
                    ...
                    B[label = 'Large scale survey (n=690)']
                    ...
                    fixedsize = true, fontsize = 45]
                    E[label = 
                    <Factor identification<br/>Scree plot: 6 factors<br/>Minimum average partial method: 5 factors<br/>Hull method: 5 factors>
                    ]
                    ...
                    8[label = <Measurement invariance analysis (n=262)<br/>Residual measurement invariance achieved>]
                    9[label = <Semantic analysis (n=262):<br/>Semantic scale network analysis<br/>Readbility: grade level 3.33>]
...
Participants indicated filling out the online survey from a diverse range of geographic locations. Figure \@ref(fig:partmap) depicts the top ten countries with highest participation in our survey. For a complete list of geographic locations, see Suppl. Table \@ref(tab:tztable).

```{r partmap, fig.cap="Top ten countries with highest responses in our survey", out.height= '100%', out.width= '100%'}
...
colnames(descriptives) <- c("Items", "Mean", "SD", "Skew", "Kurtosis", "SW Statistics", "Item Total Correlation")
...
descriptives2 %>% 
apa_table(landscape =T,caption ="Descriptive Statistics", escape =F, font_size = "footnotesize", note = "*p<.001", align = c("p{1cm}","p{10cm}", rep("p{1cm}", ncol(descriptives2)))) 
```
...
# header
  tab_header(
    title = md("**LEBA**"),
    subtitle = md("Summary Descriptives CFA Sample (Nn=262)")
  ) %>% 
  tab_footnote(
    footnote = md("**Histogram**"),
    locations = cells_column_labels(columns = hist_data)
...
apa_table(Invariance.table, align = "c",  caption = "Measurment Invariance analysis on CFA sample (n=262) across native and non-native English speakers.", note = " a = Metric vs Configural; b = Scalar vs Metric; c = Residual vs Scalar; d = Structural vs Residual;* =  df of model comparison", landscape = T, font_size = "footnotesize" )
...
Table \@ref(tab:itemfittab) summarizes the item fit indexes of the items. All of the items had RMSEA value $\le$.06 indicating adequate fit of the items. Sup.Figure \@ref(fig:personfit) depicts the person fit Zh statistics histogram of out fitted models. Zh statistics are larger than -2 for most participants, suggesting a good person fit of the selected IRT models. 
...
LEBA can be used to profile individuals based on their light exposure related behaviours, which can facilitate the development process of individual interventions to promote health. All the five factors of LEBA can identify 'problematic' behaviours that are opposed to good light hygiene. 
