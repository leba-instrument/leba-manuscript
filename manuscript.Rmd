...
## Merge "0"s and "1"s into "1"s 9select,subset assign)
...
as_kable(format = 'pipe') -> desc_kable #save it as a kable
...
Table 1 summarizes the demographic characteristics of the online survey participants. Anonymous online questionnaire responses from a total of *n* = `r nrow(desctable.data)` participants were included in the analysis of the current study, split into samples for exploratory (EFA: *n* = `r nrow(desctable.data[desctable.data$IncludedInCFA == "1. EFA Sample",])`) and confirmatory factor analysis (CFA: *n* = `r nrow(desctable.data[desctable.data$IncludedInCFA == "2. CFA Sample",])`). Only participants completing the full LEBA questionnaire were included, thus there are no missing values in the item analyses. XX participants were excluded from analysis due to not passing at least one of the "attention check" items. The EFA sample included participants filling out the questionnaire from XX.XX.XXXX to YY.YY.YYYY, whereas participants who filled out the questionnaire from ZZ.ZZ.ZZZZ to 03.09.2021 were included in the CFA analysis. Participants reported filling out the online survey from a diverse range of geographic locations. The four most common geographic locations included:
...
reliability2 <- semTools::reliability(fit.Cor.2, return.total = T)
## Summary of Model 
...
```{r IRT data, eval=FALSE, include=FALSE}
...
## F1 Item Fit ####
F1_itemfit <- itemfit(F1_fit,fit_stats = c("S_X2", "G2","Zh", "infit"),
                                  impute=10)


F1.missfit <- F1_itemfit[F1_itemfit$p.S_X2 < 0.05, ]


## F2 Item Fit ####
F2_itemfit <- itemfit(F2_fit,fit_stats = c("S_X2", "G2","Zh", "infit"),
                      impute=10)


F2.missfit <- F2_itemfit[F2_itemfit$p.S_X2 < 0.05, ]


## F3 Item Fit ####
F3_itemfit <- itemfit(F3_fit,fit_stats = c("S_X2", "G2","Zh", "infit"),
                      impute=10)


F3.missfit <- F3_itemfit[F3_itemfit$p.S_X2 < 0.05, ]

## F4 Item Fit ####
F4_itemfit <- itemfit(F4_fit,fit_stats = c("S_X2", "G2","Zh", "infit"),
                      impute=10)


F4.missfit <- F4_itemfit[F4_itemfit$p.S_X2 < 0.05, ]

## F5 Item Fit ####
F5_itemfit <- itemfit(F5_fit,fit_stats = c("S_X2", "G2","Zh", "infit"),
                      impute=10)


F5.missfit <- F5_itemfit[F5_itemfit$p.S_X2 < 0.05, ]
...
F1_iteminfo <- plot(F1_fit, type = "infotrace", main = "Blue Filter Item Information")
...
F2_iteminfo <- plot(F2_fit, type = "infotrace", main = "Natural Light Item Information")
...
F3_iteminfo <- plot(F3_fit, type = "infotrace", main = "Smart Device Item Information")
...
F4_iteminfo <- plot(F4_fit, type = "infotrace", main = "Sleep Environment Item Information")
...
F5_iteminfo <- plot(F5_fit, type = "infotrace", main = "Electric Light Item Information")

## Plot grid ####
library(cowplot)
plot_grid(F1_OCC, F2_OCC,F3_OCC,F4_OCC,F5_OCC, 
          labels = c('A', 'B','C', 'D', 'E' ), label_size = 10)

plot_grid(F1_info, F2_info,F3_info,F4_info,F5_info, 
          labels = c('A', 'B','C', 'D', 'E' ), label_size = 10)
