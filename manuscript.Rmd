...
F1.iic2 <- ggplot(F1.item.info.data.2,aes(Theta,value,col=variable))+geom_line()+apatheme+
  ...
  labs(y = "Item Information")+facet_wrap(~variable)+theme(strip.text.x = element_text(size = 15))+geom_hline(yintercept = .20, colour ="red")+scale_y_continuous(breaks=c(5,50,100,200), limits = c(0, 200))+geom_area()+ scale_color_npg()+aes(fill = as.factor(variable))
