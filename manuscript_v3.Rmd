...
Response patterns for the entire sample (n=`r nrow(sem.data)`) are depicted in Figures \@ref(fig:efagtPic1) and \@ref(fig:efagtPic2). All items violated univariate [@shapiroAnalysisVarianceTest1965] and multivariate normality [(@mardiaMeasuresMultivariateSkewness1970]. The multivariate skewness was`r printnum(mardia.all.data$b1p)` (p<0.001) and the multivariate kurtosis was `r printnum(mardia.all.data$b2p)` (p<0.001). 
...
**Supplementary Figure 1** depicts the univariate descriptive statistics for the EFA sample (n=`r nrow(EFA.data)`). Likewise, our data violated the both univariate [@shapiroAnalysisVarianceTest1965] and multivariate normality assumptions [@mardiaMeasuresMultivariateSkewness1970]. The multivariate skew was `r printnum(mardia$b1p)` (p<0.001) and the multivariate kurtosis was `r printnum(mardia$b2p)` (p <0.001). The corrected item-total correlation ranged between `r apa(low.r.corec,2,T)` and `r apa(high.r.corec,2,T)`. However, no item was discarded based on descriptive statistics or item analysis.
...
While investigating the optimum factor number for the LEBA inventory, the Scree plot ( Figure \@ref(fig:efa-plot-print)-B) revealed a six-factor solution, whereas the minimum average partial (MAP) method [@velicerDeterminingNumberComponents1976] (**Supplementary Table 3**) and Hull method [@lorenzo-sevaHullMethodSelecting2011] ( Figure \@ref(fig:efa-plot-print)-C) implied a five-factor solution. Hence, we tested both five-factor and six-factor solutions using iterative EFA where we gradually identified and discarded problematic items (factor-loading <0.30 and cross-loading >0.30).
