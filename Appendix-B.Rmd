...
```{r AEFAassumptions, include=FALSE}
...
```{r AfigCor, fig.align='center', fig.cap="Correlation plot of the items",  out.width="100%", include=FALSE,}
...
```{r AMAPefa, include=FALSE}
#MAP
psych::VSS(EFA.data, rotate = "varimax", fm = 'minres', n.obs =428 )
```

```{r AfacIdFig, out.height="200%", out.width="100%",fig.align='center', fig.cap='Factor Identification (A) Parallel analysis (B) Scree Plot',  warning=FALSE, results= 'asis'}
cowplot::plot_grid(parallel, scree.plot, labels = "AUTO",ncol=1, label_size = 8,align = "v")

```

Horn's parallel analysis with 500 iterations indicated a five-factor solution. However, Scree plot and the MAP method suggested 6-factor solution. five-factor solution (Figure\\ \\\@ref:AfacIdFig). As a result, we tested both five-factor and six-factor solutions.

```{r AEFA, include=FALSE}
...
```{r Afactordetails, include=FALSE}
...
```{r AEFAResiduals, fig.align='center', fig.cap=' Histogram of residulas:  five-factor solution', warning=FALSE, include=FALSE, out.height="50%", out.width="50%"}
